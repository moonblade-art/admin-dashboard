...
const ActivityLog = () => {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      setStats({
        ...
        inserts: sanitizedData.filter(
          (l) => (l.action || '').toUpperCase() === 'INSERT'
        ).length,
        updates: sanitizedData.filter(
          (l) => (l.action || '').toUpperCase() === 'UPDATE'
        ).length,
        deletes: sanitizedData.filter(
  ...
  const columns = [
...

    {
      ...
      label: 'Old Data',
      render: (value) => (
        <pre className="text-xs bg-gray-50 p-2 rounded-lg overflow-auto max-w-[250px]">
          {formatJson(value)}
        </pre>
      ),
    },
